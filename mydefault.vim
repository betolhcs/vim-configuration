" Vim colorscheme
"
" Author: Luis Humberto Chaves Senno
...
" User Options --- {{{
if !exists('g:mydefault_italic')
  if has('gui_running') || $TERM_ITALICS == 'true'
    let g:mydefault_italic = 1
  else
    let g:mydefault_italic = 0
  endif
endif
" }}}

if g:mydefault_italic
  let s:italic = { 'cterm': 'italic', 'gui': 'italic' }
else
  let s:italic = { 'cterm': 'NONE', 'gui': 'NONE' }
endif

let s:yellow = { 'cterm': 3, 'gui': '#FFD866' }
let s:purple = { 'cterm': 5, 'gui': '#AB9DF2' }
let s:light_green = { 'cterm': 2, 'gui': '#A9DC76' }
let s:light_blue = { 'cterm': 6, 'gui': '#78DCE8' }
let s:magenta = { 'cterm': 1, 'gui': '#FF6188' }
let s:orange = { 'cterm': 4, 'gui': '#FC9867' }

" Search colours. Specifically not in the monokai palette so that they will
" stand out.
let s:black = { 'cterm': 232, 'gui': '#1A1B26' }
let s:bright_yellow = { 'cterm': 226, 'gui': 'yellow' }

" Monochrome in order light -> dark
let s:white = { 'cterm': 7, 'gui': '#FCFCFA' }
let s:light_grey = { 'cterm': 250, 'gui': '#bcbcbc' }
let s:grey = { 'cterm': 245, 'gui': '#8a8a8a' }
let s:dark_grey = { 'cterm': 59, 'gui': '#5f5f5f' }
let s:darker_grey = { 'cterm': 8, 'gui': '#444444' }
let s:light_charcoal = { 'cterm': 237, 'gui': '#2b2b2b' }
let s:charcoal = { 'cterm': 0, 'gui': '#1A1B26' }
...
call Highlight('CursorLineNR', { 'fg': s:bright_yellow, 'bg': s:charcoal, 'style': s:none })
...
call Highlight('LineNr', { 'fg': s:white, 'bg': s:darker_grey, 'style': s:none })
